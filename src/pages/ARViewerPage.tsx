...
export function ARViewerPage() {
  ...
  const startMindAR = async () => {
    ...
    try {
      ...
      // Using full URLs + @vite-ignore so Vite doesn't bundle a separate three instance.
      // The browser's import map resolves these to the same three@0.160.0 MindAR uses.
      ...
      // Load the 3D model using the CDN THREE's GLTFLoader (same instance as MindAR)
      const loader = new GLTFLoader();
      const gltf = await new Promise<any>((resolve, reject) => {
        loader.load(item.model_url!, resolve, undefined, reject);
      });
      ...
      // Do NOT set a custom render loop — MindAR's start() handles tracking + rendering.
      // Do NOT import 'three' from npm anywhere in this file — that creates a second
      // Three.js instance and the model won't render in MindAR's scene.
